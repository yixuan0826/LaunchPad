...
// One editable step of a keymouse sequence.
//
// The three step kinds share a single set of controls: `fields` describes which
// editors each kind needs, so there is no per-kind duplication.  Edits are
// written straight into `stepData`, an object owned by the editor's step array.
Frame {
    id: stepDelegate
    ...
    Flow {
        ...
        Repeater {
            ...
            delegate: Row {
                ...
                // Only the editor matching the field's kind is instantiated, so no
                // control ever binds to spec keys its own field does not carry.
                Loader {
                    anchors.verticalCenter: parent.verticalCenter
